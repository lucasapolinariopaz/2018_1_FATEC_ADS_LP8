...
namespace seguradora
{
	public partial class CadastroApolice : Form
	{
		...
		private void btn_ConsultarCorretor_Click(object sender, EventArgs e)
		{
			...
			try
			{
				if (dr.Read())
				{
					...
					txt_CorretorSenha.Text = dr["senha"].ToString();
				}
				else
				{
		...
		{
			String sql = "SELECT car.cod_car, car.marca, car.modelo, car.cor, car.ano_fabricao, car.ano_modelo, car.chassi, " +
				"car.placa, car.cod_cli, cli.nome, cli.cpf, cli.data_nasc, cli_endereco, cli_telefone" +
				" FROM carro car INNER JOIN cliente cli ON car.cod_cli = cli.cod_cli WHERE car.placa = @placa;";
			SqlConnection conn = Conexao.obterConexao();
			SqlCommand cmd = new SqlCommand(sql, conn);
		...
		private void btn_Salvar_Click(object sender, EventArgs e)
		{
			if (txt_CorretorCod.Text != "" && txt_CarroCod.Text != "" && txt_ClienteCod.Text != "" &&
				txt_ApoliceValor.Text != "" && txt_ApoliceVigencia.Text != "" && txt_ApoliceFranquia.Text != "" && 
				txt_ApoliceModelo.Text != "")
			{
				string sql = "INSERT INTO apolice(valor, vigencia, franquia, modelo, cod_corretor, cod_cli) " +
					"VALUES(@valor, @vigencia, @franquia, @modelo, @cod_corretor, @cod_cli)";
				SqlConnection conn = Conexao.obterConexao();
				SqlCommand cmd = new SqlCommand(sql, conn);
				...
				cmd.Parameters.Add(new SqlParameter("@modelo", txt_ApoliceModelo.Text));
